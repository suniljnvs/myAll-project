    ...
    try {
        ...
        if (!blog.isDeleted == true) {
            let updatedData = await blogModel.findByIdAndUpdate(
                blogId, { $set: {...fieldToUpdate } }, { new: true, upsert: true }
            );
...
const deleteBlog = async function(req, res) {
    try {
        ...
        let check = await blogModel.findOne({ _id: data, isPublished: true }, // isPublished: true is required for a blog to be deleted
            {
                isDeleted: 1,
                _id: 0,
            }
...
module.exports = { createBlog, getBlogs, updateBlog, deleteBlog, deleteBlogsQueryParams };